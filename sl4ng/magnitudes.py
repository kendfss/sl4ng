...
def magnitude(value: complex, omissions: list = "mono deca hecto".split()):
    ...
    def booly(i):
        return len(str(int(value))) < len(str(10 ** mags[i + 1]))
...
if __name__ == "__main__":
    ...
    def band(level, base=10, shift=0, root=1):
        return tuple(root + i + shift for i in ((level + 1) * base, level * base))[::-1]

    def format(selection):
        return int("".join(str(i) for i in selection))
    ...
    for size in sizes[::-1]:
        ...
        """.splitlines()
        print(
            *((x.strip(), x)[i < 1] for i, x in enumerate(string)),
            sep="\n\t",
            end="\n\n",
        )
    print(lasso(sizes[-1]))
